...
**Second Marker:** Dr. Alastair Donaldson

*Source: Imperial College London, MEng thesis, 2016.*
*This file is the cleaned `pdftotext -layout` extraction of the original PDF.*
...
3 Base Model                                                                             20
  ...
  3.4 Compatibility . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .    28
   ...
   3.15 Well-Formed Visibility . . . . . . . . . . . . . . . . . . . . . . . . . . . .     50
        ...
        3.15.5 Comparison to Pony S . . . . . . . . . . . . . . . . . . . . . . . .        54
        3.15.6 Examples . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .      54
   3.16 Well-Formed Heaps . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .      56
        3.16.1 Comparison to Pony S . . . . . . . . . . . . . . . . . . . . . . . .        57
...
system of capabilities to include ephemeral modifiers in section 3.3.1 which were previously considered separately from the capabilities themselves. We show that we can now
...
different and otherwise unrelated capability).
    Using these new temporaries we then proceed to prove that our property of well-formed visibility is preserved across execution of an expression in section 4.3, proving a
...
## 2.5 Structural and Nominal Subtyping
 ...
 1 trait Named                                         1 trait Named
 2   fun name(): String val =>                         2   fun name(): String val =>
 3     "Somebody"                                      3     "Somebody"
 4                                                     4

 5   class Alice is Named                              5 class Bob
 6                                                     6   fun name(): String val =>
 7                                                     7     "Bob"
 8                                                     8

 9 class AliceUsage                                    9 class BobUsage
10   fun use_named(x: Named ref) =>                   10   fun use_named(x: Named ref) =>
11     ...                                            11     ...
12                                                    12

13     fun call_with_alice() =>                       13   fun call_with_bob() =>
14       var alice = Alice.create()                   14     var bob = Bob.create()
15       this.receive_named(alice)                    15     this.receive_named(bob)
                                                      16     // ˆ fails to compile
...
## D.1 Basic Definitions
 ...
 1   % Define the six basic capabilities (κ, see section 3.3)
 2   capability_kappa(K) :- member(K, [iso, trn, ref, val, box, tag]).
 3

 4 % Extended capabilities with ephemerals (λ, see section 3.3.1)
...
14 % recovery (R(λ), see section 3.9)
...
19 % safe to write (λ / λ0 , see section 3.8)
...
27 % local compatibility (λ ∼` λ0 , see section 3.4.1)
28 compat_l(’iso-’, tag).
...
37 % global compatibility (λ ∼g λ0 , see section 3.4.2)
...
43 % static compatibility (λ ∼s λ0 , see section 8.8.1)
...
47 % subtyping of capabilities (λ ≤ λ0 , see section 3.10)
...
72 % aliasing of capabilities (+λ, see section 3.5)
73 alias(’iso-’, iso).
74 alias(iso, tag).
...
79 % unaliasing of capabilities (−λ, see section 3.6)
80 unalias(iso, ’iso-’).
81 unalias(trn, ’trn-’).
...
84 % non-extracting viewpoint adaptation (λ . κ, see section 3.11.2)
...
5   unalias(K1oK2, X),
6   setof(Y, K1xˆ(unalias_or_id(K1, K1x),
...
2    capability_lambda(K1),
 ...
 8   alias(L1u, L1ua),
 9   compat_l(K1aa, L1),
10   compat_l(K1aa, L1u),
11   compat_l(L1a, K1a),
12   compat_l(L1ua, K1a),
13   subtype(K1, K1s),
14   alias(K1s, K1sa),
15   unalias_or_id(K1sa, K1sax),
16   alias(K1sax, K1saxa),
17   \+((compat_l(K1saxa, L1), compat_l(K1saxa, L1u),
18     compat_l(L1a, K1sax), compat_l(L1ua, K1sax))).
...
5   alias(Ka, Kaa),
6   alias(Kx, Kxa),
7   \+((compat_l(Kaa, Kx), compat_l(Kxa, Ka))).
